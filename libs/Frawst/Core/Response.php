		...
		public function redirect($to = null, $status = self::STATUS_FOUND, $external = false) {
			...
			if (!$external) {
				...
				if ($this->request->isAjax()) {
					$to .= \Frawst\AJAX_SUFFIX;
				}
				$to = $this->Module->path($to);
			}
			...
			try {
				if(!is_string($this->data)) {
					if(isset($this->internalRedirect)) {
						$req = Module::factory($this->request()->module()->name())
						     ->request($this->internalRedirect, array(), 'GET', $this->request->header());
