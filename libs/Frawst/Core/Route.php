<?php
	...
	class Route {
		...
		public static function resolve($module, $route, $routeCustom = false) {
			...
			if(View::exists($module, $content = strtolower($route))) {
				$template = $content;
				$c = str_replace(' ', '/', ucwords(str_replace('/', ' ', $route)));
				if(Controller::exists($module, $c)) {
					$controller = $c;
				} else {
					$controller = null;
				}
				...
				while($exists && $abstract && count($parts)) {
					...
					if($parts[0] != '' && Controller::exists($module, $c)) {
						array_shift($parts);
						$controller = $c;
						$abstract = Controller::isAbstract($module, $controller);
					} else {
						$exists = false;
					}
				}
				...
				if($controller !== null) {
					$params = $parts;
					if(View::exists($module, $c = strtolower($controller))) {
						$template = $c;
					}
				}
			}
